package com.game.hero_datasource.cache

import com.game.hero_domain.Hero
import com.game.hero_domain.HeroRole
import com.game.herodatasource.cache.HeroDbQueries
...
class HeroCacheImpl(
    ...
    override suspend fun searchByName(localizedName: String): List<Hero> {
        return queries.searchHeroByName(localizedName).executeAsList().map {
            it.toHero()
        }
    }

    override suspend fun searchByAttr(primaryAttr: String): List<Hero> {
        return queries.searchHeroByAttr(primaryAttr).executeAsList().map {
            it.toHero()
        }
    }
